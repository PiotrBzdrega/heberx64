...
void TestHopperCoinRelease( FireFlyUSB * FireFly )
{
	...
	while ( TRUE )
	{
		...
		if ( key == 'R' || key == 'r' )
		{
			...
			{
				if ( FireFly->GetLastError( ) == USB_MESSAGE_COIN_HOPPER_BUSY )
				{
					printf( "The coin handler is busy\n\n" );
				}
				else if ( FireFly->GetLastError( ) == USB_MESSAGE_COIN_HOPPER_INIT_JAM )
				{
					printf( "The counting input line is already ACTIVE. This should be inactive before\n" );
					printf( "starting to releasing coins.\n\n" );
				}
				else
				{
					printf( "ReleaseParallelHopperCoins returned error  %d.\n\n", (int)FireFly->GetLastError( ) );
					return;
				}
			}
		}
	...
	//			return;
			...
			else if ( status == Stopping ) printf( "Stopping, %d released.\n", coinsReleased );
